...
# Workaround for crontab not running because of Docker hardlinking issue: touch cron.d/* files
echo 'Running hardlink workaround for "/etc/cron.d/*\".'
find /etc/cron.d/* -type f -exec touch {} \;

# Empty /etc/cron.* directories notice
if [ ! "$(ls -A /etc/cron.hourly)" ] && [ ! "$(ls -A /etc/cron.daily)" ] && [ ! "$(ls -A /etc/cron.monthly)" ] && [ ! "$(ls -A /etc/cron.weekly)" ]; then
	echo 'Notice: No executable files found under "/etc/cron.hourly", "/etc/cron.daily", "/etc/cron.monthly", "/etc/cron.weekly". You can copy or mount executables to these locations.'
fi

# Empty /etc/cron.d directory notice
if [ ! "$(ls -A /etc/cron.d)" ]; then
	echo 'Notice: No crontab files found under "/etc/cron.d". You can copy or mount custom crontab files here.'
fi

# Files with extensions in /etc/cron.* directories notice
if [ "$(find /etc/cron.hourly -maxdepth 1 -type f -name '*.*' 2>/dev/null)" ] \
	|| [ "$(find /etc/cron.daily -maxdepth 1 -type f -name '*.*' 2>/dev/null)" ] \
	|| [ "$(find /etc/cron.monthly -maxdepth 1 -type f -name '*.*' 2>/dev/null)" ] \
	|| [ "$(find /etc/cron.weekly -maxdepth 1 -type f -name '*.*' 2>/dev/null)" ]; then
	echo 'Warning: Debian ignores executable file(s) with extension(s) found in "/etc/cron.hourly", "/etc/cron.daily", "/etc/cron.monthly" or "/etc/cron.weekly" directories. Your executable files in these directories should not have extensions.(.sh, etc...)'
fi
